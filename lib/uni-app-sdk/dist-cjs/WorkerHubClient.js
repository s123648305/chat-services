...
exports.WorkerHubClient = void 0;
const UniSocketTransport_js_1 = require("./transport/UniSocketTransport.js");
const UniRequestTransport_js_1 = require("./transport/UniRequestTransport.js");
...
class WorkerHubClient {
    constructor(options) {
        ...
        this.transport = ((_d = options.transportFactory) !== null && _d !== void 0 ? _d : UniSocketTransport_js_1.createUniSocketTransport)({
            url: options.relayUrl,
            headers: authHeaders,
        });
        this.requestTransport = ((_e = options.requestFactory) !== null && _e !== void 0 ? _e : UniRequestTransport_js_1.createUniRequestTransport)({
    ...
    async createSession(input = {}) {
        ...
        if (input.sessionKey && session.sessionKey !== input.sessionKey && session.key !== input.sessionKey) {
            throw new types_js_1.WorkerHubError('SESSION_KEY_MISMATCH', 'sessions.create returned a different sessionKey.');
        }
        return session;
    }
    ...
    sendFrame(requestId, frame, timeoutMs) {
        const promise = new Promise((resolve, reject) => {
            const timer = setTimeout(() => {
                ...
                reject(new types_js_1.WorkerHubError('REQUEST_TIMEOUT', `Request ${requestId} timed out.`));
            }, timeoutMs);
    ...
    handleRelayResponse(frame) {
        ...
        const error = isRecord(frame.error)
            ? new types_js_1.WorkerHubError((_a = stringValue(frame.error.code)) !== null && _a !== void 0 ? _a : 'RELAY_ERROR', (_b = stringValue(frame.error.message)) !== null && _b !== void 0 ? _b : 'Relay request failed.', frame.error)
            : new types_js_1.WorkerHubError('RELAY_ERROR', 'Relay request failed.');
        this.rejectPending(requestId, error);
    }
    handleRelayError(frame) {
        var _a, _b;
        const error = isRecord(frame.error)
            ? new types_js_1.WorkerHubError((_a = stringValue(frame.error.code)) !== null && _a !== void 0 ? _a : 'RELAY_ERROR', (_b = stringValue(frame.error.message)) !== null && _b !== void 0 ? _b : 'Relay error.', frame.error)
            : new types_js_1.WorkerHubError('RELAY_ERROR', 'Relay error.');
        const requestId = stringValue(frame.requestId);
    ...
    rejectAll(code, message) {
        for (const [requestId, pending] of this.pending) {
            clearTimeout(pending.timer);
            pending.reject(new types_js_1.WorkerHubError(code, message));
            this.pending.delete(requestId);
        }
    }
}
exports.WorkerHubClient = WorkerHubClient;
...
function readWorkerChatSession(value) {
    var _a;
    if (!isRecord(value)) {
        throw new types_js_1.WorkerHubError('INVALID_SESSION_RESPONSE', 'sessions.create returned invalid data.');
    }
    const sessionKey = (_a = stringValue(value.sessionKey)) !== null && _a !== void 0 ? _a : stringValue(value.key);
    const key = sessionKey;
    if (!key) {
        throw new types_js_1.WorkerHubError('INVALID_SESSION_RESPONSE', 'sessions.create response is missing sessionKey.');
    }
...
function readSendMessageResult(value) {
    if (!isRecord(value)) {
        throw new types_js_1.WorkerHubError('INVALID_CHAT_RESPONSE', 'chat.send returned invalid data.');
    }
    const runId = stringValue(value.runId);
    const status = stringValue(value.status);
    if (!runId || !status) {
        throw new types_js_1.WorkerHubError('INVALID_CHAT_RESPONSE', 'chat.send response is missing runId or status.');
    }
    return { runId, status };
}
